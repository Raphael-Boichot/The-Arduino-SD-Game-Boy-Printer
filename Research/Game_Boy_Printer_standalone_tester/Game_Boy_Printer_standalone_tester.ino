...
void setup() {
  ...
  transmit_data_packets_320();  //packet formatting
  transmit_data_packets_640();  //packet formatting
  transmit_data_packets_320();  //packet formatting
  transmit_data_packets_320();  //packet formatting
  transmit_data_packets_320();  //packet formatting
  transmit_data_packets_640();  //packet formatting
...
//dev notes: you can use packets of less than 640 bytes, BUT when printing command is sent, any bytes to the next multiple of 640 will be a buffer overflow
//this means than sending two consecutive 320 bytes packets before printing is legit, but only one will print garbage. I guess this can be generalized to smaller packets
void transmit_data_packets_320() {
  Serial.println();
