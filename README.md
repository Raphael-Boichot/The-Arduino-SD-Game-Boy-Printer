...
When using the GB printer with actual games, the palette can be freely chosen to match the 2-bits encoded grayscale value (between 0 and 3) with the 2-bits darkness levels (0 = white, 3 = black). Games can even in some case uses a two colors palette (on purpose) or a different palette at each print command. There is just one particular value : 0x00 which is the default palette (in this case the 2-bits encoded grayscale corresponds to the 2-bits encoded printing intensity). Palette 0x00 has the same effect than palette 0xE4 (the mainly encountered palette in games) but 0x00 is used only in the non released Pokémon Picross game recently leaked (september 2020). There is even one game with a wrong palette : in Disney's Tarzan on GBC, palette must be 0xD2 instead of 0xE1 (at offset 0xf8c91 of the french rom i.e.). This wrong palette creates an inversion between light gray and dark gray, barely visible on paper but very obvious on screen. I use here the classical 0xE4 palette for my code.
