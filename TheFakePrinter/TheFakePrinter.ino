...
char INQU[]="88 33 0F 00 00 00 0F 00 00 00"; //INQUIRY, NOT USED HERE
...
void setup() {
 ...
 Serial.println(' ');
   ...
   pos=0;
    ...
    checksum=130+4;
    ////////////////////here we start readind into the file on SD card
    for (int i = 1; i <=640; i++) {
...
Serial.print(packet_number);
 ...
 Serial.println(' ');
  ...
  Serial.println(' ');
  Serial.println("Printing command");// here we send the printing command
  pos=0;
    for (int i = 1; i <=14; i++) {
    character=PRNT[pos];
    Serial.print(character);
  ...
  Serial.println(' ');
 delay(1500);// Delay between two images must be not to short, I did not check the minimal time
  }
dataFile2.close();
